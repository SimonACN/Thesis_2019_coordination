...
ros::Subscriber local_pos = nh.subscribe("mavros/local_position/pose", 1,positionCallback);
	...
	float x_goals[MAX_GOALS] = {0, 0, 0, 0};
	float y_goals[MAX_GOALS] = {10, 10, 10, 10};
    geometry_msgs::PoseStamped poseBefore[MAX_GOALS];
	geometry_msgs::PoseStamped pose[MAX_GOALS];
	for (int i = 0; i < MAX_GOALS; i++)
	{
		poseBefore[i].pose.position.x = x_goals[i];
		poseBefore[i].pose.position.y = y_goals[i];
		poseBefore[i].header.frame_id = "uav0/local_origin";
		poseBefore[i].pose.orientation.w = -1;
		try{
       listener.transformPose("uav1/local_origin",poseBefore[i],pose[i]);
